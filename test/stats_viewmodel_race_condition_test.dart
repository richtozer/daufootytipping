...
void main() {
  group('StatsViewModel Race Condition Tests', () {
    ...
    test('SHOULD EXPOSE BUG: _handleEventLiveScores assigns entire Scoring object when game.scoring is null', () {
      // This test simulates the exact scenario in StatsViewModel._handleEventLiveScores()
      
      // Step 1: Game starts with no scoring data
      expect(testGame.scoring, null, reason: 'Game should start with no scoring');
      ...
      if (testGame.scoring == null) {
        testGame.scoring = scoringFromLiveUpdate; // ❌ BUG: Assigns object with null official scores
      } else {
        testGame.scoring?.croudSourcedScores = scoringFromLiveUpdate.croudSourcedScores;
      }
      
      // Step 4: Verify the bug - official score fields are now explicitly null
      expect(testGame.scoring?.homeTeamScore, null, reason: 'Live update set official scores to null');
      expect(testGame.scoring?.awayTeamScore, null, reason: 'Live update set official scores to null');
      expect(testGame.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores should be present');
      ...
      expect(result, GameResult.b, reason: 'Home wins 18-12, should be GameResult.b');
    ...
    test('SHOULD FAIL INITIALLY: Demonstrate the correct fix for data isolation', () {
      ...
      if (testGame.scoring == null) {
        testGame.scoring = Scoring(
          croudSourcedScores: scoringFromLiveUpdate.croudSourcedScores,
      ...
      expect(testGame.scoring?.homeTeamScore, null, reason: 'Official scores untouched by live data');
      expect(testGame.scoring?.awayTeamScore, null, reason: 'Official scores untouched by live data');
      expect(testGame.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores preserved');
      ...
      expect(testGame.scoring?.homeTeamScore, 10, reason: 'Official scores work perfectly');
      expect(testGame.scoring?.awayTeamScore, 8, reason: 'Official scores work perfectly');
      expect(testGame.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores preserved');
      ...
      expect(homeScore, 10, reason: 'Uses official score despite live score being 25');
      expect(awayScore, 8, reason: 'Uses official score');
      expect(result, GameResult.b, reason: 'Home wins 10-8');
    });

    test('SHOULD EXPOSE BUG: Official scores arriving first, then live scores with explicit nulls', () {
      ...
      expect(testGame.scoring?.homeTeamScore, 20, reason: 'Official scores preserved');
      expect(testGame.scoring?.awayTeamScore, 16, reason: 'Official scores preserved');
      expect(testGame.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores added');
      
      // Step 5: Priority logic should still work
      final result = testGame.scoring!.getGameResultCalculated(League.nrl);
      expect(result, GameResult.b, reason: 'Home wins 20-16 using official scores');
    });

    test('SHOULD EXPOSE BUG: Timing-dependent behavior based on arrival order', () {
      ...
      gameA.scoring = gameA.scoring?.copyWith(homeTeamScore: 12, awayTeamScore: 10);
      ...
      gameB.scoring?.croudSourcedScores = liveScoring.croudSourcedScores;
      ...
      expect(resultA, resultB, reason: 'Results should be identical regardless of arrival order');
