...
void main() {
  ...
  testWidgets('requests average points when reused for a different game', (
    tester,
  ) async {
    final secondGame = Game(
      dbkey: 'nrl-11-082',
      league: League.nrl,
      homeTeam: Team(dbkey: 'nrl-home-2', name: 'Home 2', league: League.nrl),
      awayTeam: Team(dbkey: 'nrl-away-2', name: 'Away 2', league: League.nrl),
      location: 'Stadium',
      startTimeUTC: DateTime.utc(2026, 5, 11, 12),
      fixtureRoundNumber: 11,
      fixtureMatchNumber: 82,
      scoring: Scoring(homeTeamScore: 12, awayTeamScore: 24),
    );
    final firstTip = Tip(
      dbkey: 'tip-1',
      game: game,
    ...
    final secondTip = Tip(
      ...
      tipper: tipper,
      tip: GameResult.d,
      submittedTimeUTC: DateTime.utc(2026, 5, 11, 10),
    );
    final secondGameStatsRead = Completer<void>();
    var statsReadCount = 0;
    ...
    when(() => database.get()).thenAnswer((_) async {
      ...
      if (!secondGameStatsRead.isCompleted) {
        secondGameStatsRead.complete();
      }
      return _snapshot(
        exists: true,
        value: <String, Object?>{
    ...
    await secondGameStatsRead.future;
    await tester.pump();
