  ...
  group('Scoring Architecture Fix Tests', () {
    ...
    test('FIXED: Live scores never pollute official score fields', () {
      ...
      if (testGame.scoring == null) {
        ...
        testGame.scoring = Scoring(croudSourcedScores: scoringFromLiveUpdate.croudSourcedScores);
      } else {
      ...
      expect(testGame.scoring?.homeTeamScore, null, reason: 'Official scores never touched by live data');
      expect(testGame.scoring?.awayTeamScore, null, reason: 'Official scores never touched by live data');
      ...
      expect(testGame.scoring?.homeTeamScore, 15, reason: 'Official scores work flawlessly');
      expect(testGame.scoring?.awayTeamScore, 12, reason: 'Official scores work flawlessly');
      expect(testGame.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores untouched');
      
      // Step 7: Priority logic works as designed
      final result = testGame.scoring!.getGameResultCalculated(League.nrl);
      expect(result, GameResult.b, reason: 'Home wins 15-12 using official scores');
    });
    ...
    test('COMPARE: Old vs New approach architectural difference', () {
      ...
      gameNew.scoring ??= Scoring(croudSourcedScores: liveScoreData.croudSourcedScores); // ✅ Clean
      
      // Both approaches should work functionally due to priority logic
      gameOld.scoring = gameOld.scoring!.copyWith(homeTeamScore: 20, awayTeamScore: 16);
      gameNew.scoring = gameNew.scoring!.copyWith(homeTeamScore: 20, awayTeamScore: 16);
    ...
    test('VERIFY: Race condition eliminated by data isolation', () {
      ...
      for (final game in games) {
        ...
        game.scoring ??= Scoring(croudSourcedScores: liveScoreData.croudSourcedScores);
        game.scoring?.croudSourcedScores = liveScoreData.croudSourcedScores;
        
        // Verify official scores remain pristine
        expect(game.scoring?.homeTeamScore, null, reason: 'Game ${game.dbkey} official scores untouched');
        expect(game.scoring?.awayTeamScore, null, reason: 'Game ${game.dbkey} official scores untouched');
      }
      ...
      for (final game in games) {
        ...
        expect(result, GameResult.b, reason: 'Game ${game.dbkey} scoring works perfectly');
        
        // Verify data isolation is maintained
        expect(game.scoring?.homeTeamScore, 20, reason: 'Official scores preserved');
        expect(game.scoring?.croudSourcedScores?.length, 1, reason: 'Live scores preserved');
      }
    });
  });
}