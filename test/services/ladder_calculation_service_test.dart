  ...
  group('LadderCalculationService Tests', () {
    ...
    test('should return an empty ladder if no league teams are provided', () {
      final game1 = _createGame(
        dbkey: 'nrl-1-1', homeTeam: teamNrlA, awayTeam: teamNrlB, league: League.nrl, 
        startTime: DateTime.now().subtract(const Duration(days:1)), homeScore: 10, awayScore: 0
      );
      final ladder = service.calculateLadder(
        allGames: [game1],
        leagueTeams: [],
    ...
    test('should correctly calculate NRL ladder for a single home win', () {
      final game1 = _createGame(
        dbkey: 'nrl-1-1', homeTeam: teamNrlA, awayTeam: teamNrlB, league: League.nrl,
        startTime: DateTime.now().subtract(const Duration(days: 1)), homeScore: 12, awayScore: 6,
      );
      ...
      final ladderTeamB = ladder.teams.firstWhere((t) => t.teamName == 'NRL Team B');
    ...
    test('should correctly calculate AFL ladder for a single away win and a draw', () {
      final game1 = _createGame( // Away win for teamAflB
        dbkey: 'afl-1-1', homeTeam: teamAflA, awayTeam: teamAflB, league: League.afl,
        startTime: DateTime.now().subtract(const Duration(days: 2)), homeScore: 60, awayScore: 90,
      );
      final game2 = _createGame( // Draw between teamAflA and teamAflC
        dbkey: 'afl-1-2', homeTeam: teamAflA, awayTeam: teamAflC, league: League.afl,
        startTime: DateTime.now().subtract(const Duration(days: 1)), homeScore: 70, awayScore: 70,
      );
      ...
      // Team B (Won 1)
      expect(ltB.played, 1); expect(ltB.won, 1); expect(ltB.lost, 0); expect(ltB.drawn, 0);
      expect(ltB.pointsFor, 90); expect(ltB.pointsAgainst, 60);
      expect(ltB.points, 4); // AFL: 4 points for a win
      expect(ltB.percentage, closeTo(150.0, 0.01));

      // Team C (Drew 1)
      expect(ltC.played, 1); expect(ltC.won, 0); expect(ltC.lost, 0); expect(ltC.drawn, 1);
      expect(ltC.pointsFor, 70); expect(ltC.pointsAgainst, 70);
      expect(ltC.points, 2); // AFL: 2 points for a draw
      expect(ltC.percentage, closeTo(100.0, 0.01));
      
      // Team A (Lost 1, Drew 1)
      expect(ltA.played, 2); expect(ltA.won, 0); expect(ltA.lost, 1); expect(ltA.drawn, 1);
      expect(ltA.pointsFor, 130); expect(ltA.pointsAgainst, 160); // 60+70 For, 90+70 Against
    ...
    test('should ignore games without scores', () {
      final gameNoScore = _createGame(
        dbkey: 'nrl-1-1', homeTeam: teamNrlA, awayTeam: teamNrlB, league: League.nrl,
        startTime: DateTime.now().subtract(const Duration(days: 1)), // No scores
      );
      final ladder = service.calculateLadder(
