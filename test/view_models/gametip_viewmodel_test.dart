...
void main() {
  ...
  late Game
      mockUpcomingGame; // The game for which insights are being generated - use real Game with MockTeams
  ...
  group('getFormattedHistoricalMatchups', () {
    test('Correctly formats historical games with various tip types', () async {
      ...
      final histGame2 = createHistoricalGame(
        ...
        league: League.nrl, // Teams swapped for variety
      ...
      final histgame5Usertippeddraw = createHistoricalGame(
      ...
      final tip5 = createHistoricalTip(
          game: histgame5Usertippeddraw,
          tipper: mockCurrentTipper,
          tipResult: GameResult.c,
          isDefault: false); // Active Draw tip

      // Updated to call getCompleteMatchupHistory
      when(mockGamesViewModel.getCompleteMatchupHistory(
              mockUpcomingHomeTeam, mockUpcomingAwayTeam, League.nrl))
          .thenAnswer((_) async => [
                histGame1,
                histGame2,
                histGame3,
                histGame4,
                histgame5Usertippeddraw
              ]);

      when(mockTipsViewModel.findTip(histGame1, mockCurrentTipper))
          .thenAnswer((_) async => tip1);
      when(mockTipsViewModel.findTip(histGame2, mockCurrentTipper))
          .thenAnswer((_) async => tip2);
      when(mockTipsViewModel.findTip(histGame3, mockCurrentTipper))
          .thenAnswer((_) async => null);
      when(mockTipsViewModel.findTip(histGame4, mockCurrentTipper))
          .thenAnswer((_) async => tip4);
      when(mockTipsViewModel.findTip(
              histgame5Usertippeddraw, mockCurrentTipper))
          .thenAnswer((_) async => tip5);
      ...
      expect(
          result[1].winningTeamName,
          mockUpcomingHomeTeam
              .name); // mockUpcomingHomeTeam was the away team and won
      expect(result[1].winType, "Away");
      expect(result[1].userTipTeamName, isEmpty,
          reason: "Default Away tip should be ignored");
      ...
      expect(result[3].userTipTeamName, mockUpcomingHomeTeam.name,
          reason: "Default Home tip should be shown");
      ...
      expect(result[4].userTipTeamName, "Draw",
          reason: "User actively tipped Draw");
    });
    ...
    test('Skips games with missing scores', () async {
      ...
      final gameNullScores = createHistoricalGame(
        ...
        awayScore: null, // awayScore is null
      );
      // Ensure scoring object exists but awayTeamScore is null for gameNullScores
      (gameNullScores.scoring as MockScoring).homeTeamScore =
          10; // This was set
      when((gameNullScores.scoring as MockScoring).awayTeamScore)
          .thenReturn(null); // Explicitly stub awayTeamScore to be null

      // Updated to call getCompleteMatchupHistory
      when(mockGamesViewModel.getCompleteMatchupHistory(
              mockUpcomingHomeTeam, mockUpcomingAwayTeam, League.nrl))
          .thenAnswer((_) async =>
              [gameWithScore, gameNoScoringObject, gameNullScores]);

      when(mockTipsViewModel.findTip(gameWithScore, mockCurrentTipper))
          .thenAnswer((_) async => tipForScored);
      // findTip might be called for others, let them return null, they should be skipped before tip processing.
      when(mockTipsViewModel.findTip(gameNoScoringObject, mockCurrentTipper))
          .thenAnswer((_) async => null);
      when(mockTipsViewModel.findTip(gameNullScores, mockCurrentTipper))
          .thenAnswer((_) async => null);
