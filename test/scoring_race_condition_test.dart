...
void main() {
  group('Scoring Race Condition Tests', () {
    test(
      'SHOULD FAIL: Live scores arriving first should not prevent official score priority',
      () {
        ...
        final homeScore = scoringWithOfficialScores.currentScore(
          ScoringTeam.home,
        );
        final awayScore = scoringWithOfficialScores.currentScore(
          ScoringTeam.away,
        );
        final result = scoringWithOfficialScores.getGameResultCalculated(
          League.nrl,
        );
        ...
        expect(
          result,
          GameResult.d,
          reason: 'Away team wins 15-12, should be GameResult.d',
        );
      },
    );
    ...
    test('SHOULD FAIL: Live scores should not pollute official score fields', () {
      ...
      gameScoring ??=
          liveScoreData; // ❌ PROBLEM: Assigns entire object with null official scores

      // Step 3: Official scores arrive later
      gameScoring = gameScoring.copyWith(homeTeamScore: 10, awayTeamScore: 8);

      // Step 4: Verify clean separation
      expect(
        gameScoring.homeTeamScore,
        10,
        reason: 'Official home score should be preserved',
      );
      expect(
        gameScoring.awayTeamScore,
        8,
        reason: 'Official away score should be preserved',
      );
      expect(
        gameScoring.crowdSourcedScores?.length,
        1,
        reason: 'Live scores should be preserved',
      );
      ...
      expect(
        result,
        GameResult.b,
        reason: 'Home wins 10-8, should be GameResult.b',
      );
    });

    test(
      'SHOULD FAIL: Official scores arriving first should not be overwritten by live scores',
      () {
        ...
        gameScoring.crowdSourcedScores = liveScoreUpdate.crowdSourcedScores;
        ...
        expect(
          result,
          GameResult.b,
          reason: 'Home wins 21-18, should be GameResult.b',
        );
      },
    );

    test(
      'SHOULD FAIL: Data isolation - live and official scores should be completely independent',
      () {
        ...
        gameScoring ??= Scoring(
          homeTeamScore:
              null, // Should remain null - only live scores touch this
          awayTeamScore:
              null, // Should remain null - only live scores touch this
          crowdSourcedScores: liveScores,
        );
        ...
        expect(
          gameScoring.homeTeamScore,
          5,
          reason: 'Official scores independent of live',
        );
        expect(
          gameScoring.awayTeamScore,
          3,
        ...
        expect(
          homeScore,
          5,
          reason: 'Should use official score (5) not live score (100)',
        );
        expect(
          awayScore,
          3,
          reason: 'Should use official score (3) not live score (90)',
        );
      },
    );
  });
